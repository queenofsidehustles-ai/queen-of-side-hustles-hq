...
def move_deal_stage(deal_id):
    ...
    valid_stages = ["Checklist Downloaded", "Warming Up", "Course Purchased", "Hub Activated", "Active Subscriber", "VIP Client", "Churned"]
...
def dashboard_stats():
    ...
    pipeline_value = db.session.query(func.coalesce(func.sum(Deal.value), 0)).filter(
        Deal.stage.notin_(["Active Subscriber", "VIP Client", "Churned"])
    ).scalar()

    total_revenue = db.session.query(func.coalesce(func.sum(Deal.value), 0)).filter(
        Deal.stage.in_(["Active Subscriber", "VIP Client"])
    ).scalar()
